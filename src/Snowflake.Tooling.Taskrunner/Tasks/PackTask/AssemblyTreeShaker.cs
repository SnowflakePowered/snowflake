...
namespace Snowflake.Tooling.Taskrunner.Tasks.PackTask
{
    ...
    {
        internal IEnumerable<string> GetFrameworkDependencies(DirectoryInfo moduleDirectory, ModuleDefinition packageModule)
        {
            ...
            var deps = moduleDirectory.CreateSubdirectory("contents").EnumerateFiles(Path.GetFileNameWithoutExtension(packageModule.Entry) + ".deps.json").FirstOrDefault();
