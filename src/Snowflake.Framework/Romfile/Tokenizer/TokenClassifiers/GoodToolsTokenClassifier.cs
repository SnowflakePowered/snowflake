...
namespace Snowflake.Romfile.Tokenizer
{
    public class GoodToolsTokenClassifier : ITokenClassifier
    {
        public IEnumerable<StructuredFilenameToken> ClassifyBracketsTokens(IEnumerable<(string tokenValue, int tokenPosition)> tokens)
        {
            ...
            {
               if (goodToolsDumpFlags.Contains(tokenValue.Replace("?", "")))
               {
                    yield return new StructuredFilenameToken(tokenValue.Replace("?", ""),
                      FieldType.DumpInfo,
                ...
                if (tokenValue == "M" || tokenValue == "hFFE" || tokenValue == "C" || tokenValue == "S" || tokenValue == "hMxx")
                {
                    yield return new StructuredFilenameToken(tokenValue,
                      FieldType.System,
                      NamingConvention.GoodTools);
        ...
        public IEnumerable<StructuredFilenameToken> ClassifyParensTokens(IEnumerable<(string tokenValue, int tokenPosition)> tokens)
        {
            ...
            foreach ((string tokenValue, int tokenPosition) in tokens)
            {
                ...
                if (tokenValue.StartsWith("M") && tokenValue.Length == 2 && 
                    Int32.TryParse(tokenValue.Substring(1), out _))
                {
                    yield return new StructuredFilenameToken(tokenValue,
                             FieldType.Language,
                             NamingConvention.GoodTools);
                ...
                if (tokenValue.EndsWith("k") && tokenValue.Length == 3 &&
                   Int32.TryParse(tokenValue.Substring(0, tokenValue.Length - 2), out _))
                    {
                        yield return new StructuredFilenameToken(tokenValue,
                                 FieldType.DumpInfo,
                                 NamingConvention.GoodTools);
        ...
        internal static readonly IDictionary<string, string> goodToolsCountryLookupTable = new Dictionary<string, string>()
        {
            {"A", "AU"},
            {"AS", "AS"},
            {"B", "BR"},
